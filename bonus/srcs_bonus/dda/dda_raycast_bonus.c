...
/*   Updated: 2025/08/01 12:46:16 by kvalerii         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
)
{
	...
	offset = data->texture_params[wall_dir].image.addr
		+ ((int)start_y_coordinate
			* data->texture_params[wall_dir].image.line_length + texture.x
			* data->texture_params[wall_dir].image.bits_per_pixel / CHAR_BIT);
...
)
{
	...
	y_increase_step = (double)data->texture_params->height
		/ (double)wall_height;
...
void	dda(t_data *data)
{
	...
	while (x < WIDTH)
	{
		...
		dda_parameters.distance_to_wall = find_distance_to_wall(data,
				&dda_parameters,
				dda_parameters.player_cell);
		data->z_buffer[x] = dda_parameters.distance_to_wall;
